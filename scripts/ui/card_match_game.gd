...
static func open(parent: Node, mode: String, on_done: Callable) -> CardMatchGame:
	var g := CardMatchGame.new()
	g._mode = mode
	g._on_done = on_done
	parent.add_child(g)
	return g
...
func _ready() -> void:
	...
	if _deck.is_empty():
		push_warning("[CardMatchGame] 덱 생성 실패 — data/card_game.json 확인")
		_finish(false, {})
		return
...
func auto_play(interval := 1.0) -> void:
	if _cards.is_empty():
		return
	_auto = true
...
var _auto := false
var _auto_interval := 1.0

func _auto_tick() -> void:
	if not _auto or not is_instance_valid(self):
		return
	await get_tree().create_timer(_auto_interval).timeout
	if not _auto or not is_instance_valid(self) or _cards.is_empty():
		return
	if _busy:
		_auto_tick()
		return
	...
	for i in _cards.size():
		...
		if not bool(c["revealed"]) and not bool(c["done"]):
			pool.append(i)
	if pool.is_empty():
		return
	_on_pick(int(pool[randi() % pool.size()]))
	_auto_tick()
