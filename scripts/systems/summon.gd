...
##   · 🟦 **지급처 = 가방 알 탭**(사용자 확정 2026-08-07). 종전에는 둥지에 직접 생겨서
##     소환 즉시 슬롯을 차지하고 타이머가 돌기 시작했다 — 뽑기 알과 같이 **언제 부화할지
##     플레이어가 고르게** 바꿨다. 인벤 키는 `egg:<종id>#1`(아래 `EGG_ENHANCE_STEP` 주석).
##     그 결과 **연구소 알 강화 대상이 된다**(1강 → 2·3강으로 7.2/7.5까지) — 사용자가
##     이 파생을 알고 고른 선택지다.
...
## 가방에 넣을 때 붙이는 **알 강화 단계**. 소환 알은 등급이 최고치로 **확정**돼야 하는데
## (`plan()` 의 `Hatchery.GRADE_MAX`), 가방 알은 부화 시점에 `_start_hatch` 가 등급을 굴린다.
## 1강은 `EggUpgrade.hatch_grade(1)` = **7.0 확정**(위키 labwiki.pdf §2.1)이라 굴림을 대체해
## 같은 값이 나온다 ⇒ 지급처만 바뀌고 규칙은 그대로다. 표가 바뀌면 여기 한 곳만 고친다.
