...
static func build_one(in_path: String, out_path: String) -> int:
	...
	var tex_cache := {}
	var slot_path := {}   # slot 이름 -> Sprite2D 경로(슬롯 타임라인용)
	for s in data["slots"]:
		...
		spr.texture = at
		spr.material = pma
		spr.visible = bool(s.get("visible_default", true))   # 이펙트/스킨 슬롯은 평소 숨김
		frame.add_child(spr)
		slot_path[String(s["name"])] = root.get_path_to(spr)
	...
	if not data.get("animations", {}).is_empty():
		...
		for an_name in data["animations"]:
			...
			# slot 타임라인: 표시여부(visible, 이산) + 색알파(modulate, premult 페이드)
			for slot_name in an_data.get("slot_tracks", {}):
				if not slot_path.has(slot_name):
					continue
				var sp: String = str(slot_path[slot_name])
				var stt = an_data["slot_tracks"][slot_name]
				for prop in stt:
					var ti2 := anim.add_track(Animation.TYPE_VALUE)
					anim.track_set_path(ti2, "%s:%s" % [sp, prop])
					var keys2: Array = stt[prop]
					if prop == "visible":
						anim.value_track_set_update_mode(ti2, Animation.UPDATE_DISCRETE)
						anim.track_set_interpolation_type(ti2, Animation.INTERPOLATION_NEAREST)
						for key in keys2:
							anim.track_insert_key(ti2, float(key[0]), bool(key[1]))
					else:
						anim.value_track_set_update_mode(ti2, Animation.UPDATE_CONTINUOUS)
						anim.track_set_interpolation_type(ti2, Animation.INTERPOLATION_LINEAR)
						for ki2 in keys2.size():
							var key = keys2[ki2]
							var col := Color(key[1][0], key[1][1], key[1][2], key[1][3])
							anim.track_insert_key(ti2, float(key[0]), col)
							var curve2 := (String(key[2]) if key.size() > 2 else "L")
							if curve2 == "S" and ki2 + 1 < keys2.size():
								var tnext2: float = float(keys2[ki2 + 1][0])
								if tnext2 > float(key[0]) + 0.0002:
									anim.track_insert_key(ti2, tnext2 - 0.0001, col)
