extends SceneTree
## 유타칸 낮 PVE 전투 공식 회귀 테스트.
##
## 사용자 기준(2026-08-02): 각 지역 요구 레벨의 4세대 7.0등급 드래곤을 전투유형별로
## 장신구/스킬/젬 없이 3마리 편성하고 매 조우를 풀HP로 시작한다. 일반 몬스터 승률은 80% 이상,
## 던전 보스 승률은 40% 이상이어야 한다. 일부 약한 낮 일반 몬스터의 2타 처치는 허용한다.
## 혼돈의 틈새와 라이트 오브는 사용자 지시로 승률 판정에서 제외한다.
##
## 몬스터의 실제 속성을 사용한다. 속성이 있는 몬스터에는 가능한 아군 속성 중 가장 불리한
## 상성을 골라 실제 Battle.simulate 난수·회피·막기·크리·몬스터 스킬까지 통과시킨다.
...
# 혼돈의 틈새(8)는 유실된 서버 수치를 보간한 특수 소환 보스 구간이므로
# 유타칸 낮 일반 던전 성장곡선 검증에서 제외한다.
const EXCLUDED_STAGE_IDS := [8]
# 라이트 오브(72)는 사용자 지시로 탐험 승률 판정과 통계에서 완전히 제외한다.
...
func _init() -> void:
	_stages = _json("res://data/stages.json").get("stages", {})
	_table = _json("res://data/stat_table.json")
	_cfg = _json("res://data/combat.json")
	_skills = _json("res://data/skills.json")
	var trials := 20
		...
		for enemy_raw in enemies:
			...
			for type in TYPES:
				for tier in GENERATION_4_STAT_TIERS:
					...
					# 가장 빠른 정상 평타도 적 최대체력의 절반 미만이어야 한다(최소 3타).
