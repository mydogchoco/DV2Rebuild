...
OPTION = {
    ...
    # ── 마모루딕 '아티펙트 제련' (원작 `ArtifactBox` → `OptionSelectLayer` 모드 2) ──────
    #
    # 🟢 원작 배선 확정(2026-08-01):
    #   `OptionSelectLayer::requestOptionRetry` @011ec164 이 모드(`this+0x238`)로 갈린다 —
    #     · 2 → `game_lab2/regen_equip_option.hb`   (마모루딕 = 아티팩트 제련)
    #     · 1 → `game_cave/regen_equip_option.hb`   (동굴 = 기누의 동전, 동전 번호를 하나 더 보낸다)
    #   대상 목록은 `ArtifactBox::initData` @0143ec2c — `AccountManager::getEquip()` 중
    #   `6000 <= Item::getNo() < 7001` (아티팩트 번호대)만 추리고 `Equip::getRarity` 를 읽는다.
    #   ⇒ 아티팩트도 같은 `Equip` 이라 희귀도·옵션을 그대로 갖는다(우리 드롭도 이제 붙인다).
    #
    # ⚠️ 소모 재료 개수와 관통 가중치는 **서버 유실** → 자작(튜닝 노브).
    #   위키 equipment_0.pdf §2.4: "아티팩트는 마모루딕에게 가져가 아니마와 보네르로 옵션을
    #   돌릴 수 있는데, 이는 기누의 동전에 비해 **관통 옵션이 나올 확률이 높다**. 그래서 보통
    #   아티팩트로 관통 100을 맞춘다." → 관통(pure) 가중치만 크게 올린다.
    #   균등이면 각 스탯 1/9 ≈ 11%. 아래 표는 pure 를 12/(12+8) = **60%** 로 만든다.
    "artifact_smelt": {
        "items": {"anima": 3, "bonner": 3},
        "_items_authored": "⚠️ 자작 — 원작 소모량은 서버 유실. data/items.json 의 아니마/보네르.",
        "option_weights": {
            "pure": 12.0,
            "hp": 1.0, "att": 1.0, "def": 1.0, "blk": 1.0,
            "gold": 1.0, "exp": 1.0, "depure": 1.0, "accuracy": 1.0,
        },
        "_option_weights_source": (
            "위키 equipment_0.pdf §2.4 — 마모루딕 제련은 기누의 동전보다 관통이 잘 뜬다. "
            "가중치 12:1×8 ⇒ pure 60%(균등 11% 대비). 비율은 자작, 방향만 위키 확정."
        ),
    },
    "_reroll_items_note": (
    ...
    "_rarity_magic_cut": (
        "🟦 매직(1)은 **구현 대상이 아니다**(사용자 확정 2026-08-01) — 원작에서도 후기 "
        "업데이트로 사라진 등급이다. 어느 수급표에도 넣지 않으므로 게임 안에 매직 장비는 "
        "생기지 않는다. 등급 자체는 `grades`(옵션 1개)와 색표(`rarity_colors` 인덱스 1)에 "
        "남겨 둔다 — 원작 `Equip::getRarityColor` 표가 6칸이고, 표를 줄이면 인덱스가 밀린다. "
        "되살리려면 이 표에 \"1\" 항목만 추가하면 된다."
    ),
    "_rarity_no_upgrade_path": (
        "🟢 2026-08-01 — 희귀도는 **획득 시점에만 정해진다**. 원작 기누의 동전은 등급을 "
        "바꾸지 않는다: `ItemEquipSelectPopup::init(itemNo)` 이 아이템 번호로 요구 희귀도를 "
        "박고(485→레어 486→유니크 487→에픽 596/597→초월) `initData` @00ea7338 이 "
        "`getRarity(equip) == 그 값` 인 장비만 목록에 올린다. 종전 우리 `_reroll_options` 는 "
        "보유한 최고 등급 동전으로 **등급을 덮어써서** 일반 장비를 에픽으로 만들 수 있었다."
    ),
    ...
    # 강화 1회당 기존 옵션이 오르는 양.
    #
    # 🟦 사용자 확정 2026-08-01: "위키 기반으로 임의 값을 넣어라."
    # 원작 수치는 서버(`info_item_acc` 행)와 함께 유실됐고, 클라는 강화 성공 시 서버가 준
    # **옵션 문자열을 통째로 갈아 끼울 뿐**이라(`NewItemEnchantPopup` 응답 처리 @00ed1738 —
    # `setOption(새 문자열)` → `setUpGrade(+1)`) 증가폭을 복원할 근거가 코드에 없다.
    # ⇒ 위키에서 **끌어낼 수 있는 유일한 상한 앵커**로 값을 정한다:
    #
    #   위키 equipment_0.pdf §2.4: "보통 아티팩트로 **관통 100**을 맞춘다"
    #     → 관통은 장비 4칸에 나눠 붙이는 옵션이므로 칸당 도달 목표 ≈ 100 / 4 = **25**
    #   우리 `value_ranges["pure"]` 최고 롤 = 12, 옵션 1개가 받는 강화 = 5회(위키 §2.6)
    #     → 12 × (1 + x)^5 ≈ 25  ⇒  x ≈ 0.16  → 여유를 두고 **20%**
    #        (실제: 12 → 14 → 17 → 20 → 24 → 29. 최고 롤을 5강까지 밀어야 닿는 값이다)
    #
    # 종전 10% 는 근거 없는 자리값이었고, 값이 작은 옵션(1~5)에서는 `maxi(1, …)` 바닥에
    # 걸려 **사실상 전부 +1** 이라 등급 간 차이가 사라졌다.
    "enhance_step_pct": 20,
    "_enhance_step_authored": (
        "🟦 위키 앵커 기반 자작(사용자 확정 2026-08-01). 강화 1회 = 해당 옵션 값 +20%(최소 +1). "
        "앵커 = equipment_0.pdf §2.4 '아티팩트로 관통 100을 맞춘다' → 칸당 25 → 최고 롤 12를 "
        "5강(위키 §2.6 옵션당 강화 횟수)으로 밀면 닿는 배율. 원작 증가폭은 info_item_acc 행과 "
        "함께 유실 — 클라는 서버가 준 옵션 문자열을 통째로 교체할 뿐이다. 튜닝 노브."
    ),
}
