extends SceneTree
## 헤드리스 CardGame 단위 테스트 (§10 — logic 은 화면 없이 검증 가능).
## 실행: godot --headless --script res://scripts/tools/test_card_game.gd
## CardGame 은 데이터를 인자로 받는 순수 로직이라 autoload 없이 실제 json 을 직접 읽어 돌린다.

func _init() -> void:
	var fails := 0
	var cfg := _json("res://data/card_game.json")
	fails += _true("데이터 로드", not cfg.is_empty())
	var rng := RandomNumberGenerator.new()

	# 1) 위키 확정 규칙 — 8장/4기회, 3장/꽝1~2
	...
	# 2) match 덱 = 4쌍. 각 보상이 정확히 2장씩이어야 한다.
	rng.seed = 11
	for t in 40:
	...
	# 2b) 🔴 회귀: 쌍이 **서로 달라야** 한다. 예전엔 독립 추첨이라 8장 중 6장이 같은 보상인
	#     덱이 나왔고, 그러면 아무 두 장이나 짝이 되어 게임이 성립하지 않았다.
	rng.seed = 99
	var worst := 0
	for t in 60:
