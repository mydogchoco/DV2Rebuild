"""sim_matchup.gd 러너 — 임시 오토로드 등록 → 부팅 → 해제.
...
사용:  python scripts/tools/run_sim_matchup.py [--sim-a=999] [--sim-b=<닉>] [--sim-n=50] [--sim-mode=team|single] [--sim-dump=1]
       인자는 그대로 게임에 전달된다(기본 = 선대군 999 vs 랭커 전원 · 3vs3 · 각 200판).
"""
...
LINE = 'SimMatchup="*res://scripts/tools/sim_matchup.gd"\n'
...
def main() -> int:
    ...
    try:
        ...
        r = subprocess.run(
            [str(GODOT), "--headless", "--path", str(REPO), "--quit-after", "6000",
             "--"] + [a for a in sys.argv[1:] if a.startswith("--sim-")],
            capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=3600)
        out = (r.stdout or "") + (r.returncode and "" or "")
        # 시뮬레이터가 찍은 줄과 진짜 에러만 남긴다(부팅 로그가 길다).
        for line in out.splitlines():
            if line.startswith("[sim]") or "SCRIPT ERROR" in line or "Parse Error" in line:
                print(line)
        ...
        if "[sim]" in out:
            return 0
        if "[sim]" not in out:
            print("시뮬레이터가 아예 실행되지 않았다 — 아래는 마지막 출력 20줄:")
