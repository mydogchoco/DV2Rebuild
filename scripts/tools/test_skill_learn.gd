extends Node
## 헤드리스 검증 — 스킬 **순차 학습** 규칙 (§8 logic 계층, 화면 없이 검증).
##
## 원작 근거:
##   · `Dragon::checkSkillList(no, level)` (`docs/ref/orig_code/decomp/Dragon.c:815`, true = **불가**)
##       - level == 1  → 그 번호가 풀에 있으면 1
##       - level >= 2  → 풀에 `getNo()==no && getLevel()==level-1` 이 있어야 0
##   · `BagPopup::onClickConfirm` case 4 (0xdf5650) — 위가 참이면 `CaveSkillUseMsg1`
##       "드래곤이 이미 배웠거나 아직 배울 수 없는 스킬입니다." 팝업만 띄우고 서버 요청도 안 간다
##   · `<ToolTipDragonSkillExplain>` — "스킬은 1등급 부터 순차적으로 강화하여야 하며
##       한단계 높은 같은 스킬 스크롤을 이용하여 최대 5등급까지 강화 가능합니다."
##
## `--script` 모드가 이 환경에서 멈추므로([[dv2-godot-runtime-and-validation]]) 오토로드로 붙여
## 부팅 경로에서 돌린다:
##   project.godot [autoload] 에 `TestSkillLearn="*res://scripts/tools/test_skill_learn.gd"`
...
# ── ① can_learn 진리표 = 원작 checkSkillList 의 반대 ─────────────────────────
func _can_learn() -> int:
	var fails := 0
	_say("[게이트] 순차 학습")
	var sid := 11
	...
	fails += _true("미보유 → Lv.3 불가(사용자 지적 사례)", not L.can_learn([], sid, 3))
...
# ── ② learn_from_item 이 실제로 막고, 순차로는 5까지 오른다 ──────────────────
func _learn_flow() -> int:
	...
	# 만렙 초과: clamp(6→5) 뒤 can_learn(보유 5 != 5-1) 이 막는다.
	...
	# 다른 스킬은 그대로 Lv.1 부터 들어간다(풀 상한 없음).
...
# ── ③ 레벨 10·25·45 자동 습득은 게이트와 어긋나지 않는다(항상 미보유 Lv.1) ───
func _grants() -> int:
	var fails := 0
