...
CUSTOM: dict[int, dict] = {
    ...
    777: {  # 성좌의 주권자 — 한울
        ...
        "partial": ["크리티컬 발동 시 상대 방어율과 회피율의 절반을 무시 — 우리 판정 순서가 "
                    "**회피 → 막기 → 크리** 라, 크리가 정해지는 시점엔 회피·막기가 이미 끝나 있다. "
                    "적용하려면 전투 판정 순서를 바꿔야 한다(93 태양의 불꽃도 같은 이유로 미이식)."],
    ...
    700: {  # 각성하는 의지 — 도감 700
        ...
        "partial": ["크리티컬 발동 시 상대 방어율과 회피율의 절반을 무시 — 우리 판정 순서가 "
                    "**회피 → 막기 → 크리** 라, 크리가 정해지는 시점엔 회피·막기가 이미 끝나 "
                    "있다. 적용하려면 전투 판정 순서를 바꿔야 한다(93·777 과 같은 이유)."],
...
# ═══════════════════════════════════════════════════════════════════════════
C_WHY = {
    41: "공격 스킬 발동 시 상대 게이지/스탯 감소 — 스킬 발동 훅",
    78: "우리 엔진은 **스킬 피해에 막기를 적용하지 않는다**(막기는 평타 경로에만 있다) — "
        "이 스킬을 켜도 이득이 0이다. 스킬에도 막기를 넣을지 먼저 정해야 한다",
    60: "선제 공격(턴 순서 고정) — 주도권 계산 훅",
    91: "상대 각성스킬 무효화 — 각성스킬 간 상호작용",
}
D_WHY = {
    25: "적중마다 3회 한정 — 공격 훅 + 횟수 상태",
    26: "체력 20% 이하 트리거, 다음 3회 피해 1 — 체력 훅 + 카운터",
    35: "사망 시 아군에게 3회 피해 1 고정 — 사망 훅 + 카운터",
    46: "체력 50% 이하 흡혈 5회 — 체력 훅 + 카운터",
    52: "피격마다 **다음 피해 계수**를 누적하는 형태 — 지금 누적기(_stack_up)는 스탯만 "
        "다룬다. 피해 계수(dmg_taken) 누적을 추가하면 된다. 훅 자체는 이미 있다",
    65: "막기 성공마다 누적 → 공격 시 회복 — 막기/공격 훅",
    69: "드래곤 등급 + 상대 사망마다 배율 증가 — 사망 훅",
    75: "준 피해 누적 → 방어 시 방출 — 공격/피격 훅 + 누적",
    79: "모든 공격이 연속공격 — 행동 선택 훅",
    93: "크리 발동 시 방어율·회피율 절반 무시 — 우리 판정 순서가 회피→막기→크리라 "
        "크리가 정해질 때는 이미 둘 다 끝나 있다. 판정 순서를 바꿔야 한다",
}
