...
## 전투 오디오 회귀 검사.
## 실행: godot --headless --path . res://scenes/test_battle_audio.tscn
...
func _run_audio_test() -> void:
	# Cutin::show 원작 순서: effect_cut_in 즉시, voice_critical은 0.05초 뒤.
	...
	_expect(not immediate.any(func(p: String): return p.ends_with("voice12.mp3")),
		"critical: voice started before Cutin::show voice delay")
	await get_tree().create_timer(DragonCutin.VOICE_DELAY + 0.04).timeout
	...
	# 드래곤 피격만 damaged 음원을 사용하고, 크리티컬 피격도 생략하지 않는다.
	...
	# 2번 음원은 원작 폴더 폴백으로도 실제 로드되어야 한다.
