extends SceneTree
## 헤드리스 Titles 단위 테스트 (§8). 원작 근거: AchieveTitleLayer / info_title_v.
## 실행: godot --headless --path . --script res://scripts/tools/test_titles.gd --quit-after 3
const T := preload("res://scripts/systems/titles.gd")

func _init() -> void:
	var fails := 0
	var table = JSON.parse_string(FileAccess.open("res://data/titles.json", FileAccess.READ).get_as_text())
	var list: Array = table["titles"]
	fails += _eq("칭호 149종", list.size(), 149)
	# 모든 칭호에 아트 프레임과 획득조건이 있다.
	var bad := 0
	for t in list:
