...
EXCLUSIVE = {
    ...
    # 정의 = build_equipment.py EXCLUSIVE_EXTRA. 네 조항 전부 위키 장비에 선례가 있어
    # **새 op 없이** 기존 어휘로 옮겼다(그 선례를 각 줄에 적어 둔다).
    "샛별의 날개장식": {           # 666 샛별
        # 1) 레벨합 × 4 만큼 받는 대미지 감소 + 철갑방패와 중첩 — 불나래의 불꽃구슬과 같은 문구.
        #    정액인 근거도 같다: 원문이 중첩 가능하다고 못 박은 [철갑 방패](11)가
        #    "10+스킬레벨*5 **피해 감소**" 로 정액이다. 중첩은 저절로 된다(출처별로 쌓인다).
        # 2) 아군이 주는 데미지 30% 증가 — 노웨마의 갈기(10%)와 같은 축.
        "ops": [op("dmg_taken_flat",
                   **{"from": {"stat": "skill_level_sum", "ratio": 4.0}}),
                op("dmg_deal", pct=30, to="ally")]},
    "한울의 불꽃": {              # 777 한울
        # 1) 스킬 발동 횟수 +2 — 그림을 빌려 온 라 솔라의 불꽃과 같은 문구.
        # 2) 크리티컬 발동 시 상대의 현재 방어력 절반 무시 — 엔투라스의 불꽃 주먹과 같은 문구
        #    (`crit_pen` = 크리일 때만 관통에 더한다. Battle._hit_damage).
        "ops": [op("skill_uses", value=2),
                op("crit_pen", pct=50)]},
