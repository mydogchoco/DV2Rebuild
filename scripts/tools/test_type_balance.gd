...
func _init() -> void:
	_dragons = _load(_data_file("dragons.json"))
	_stat_table = _load(_data_file("stat_table.json"))
	_gems = _load(_data_file("gems.json"))
	_equipment = _load(_data_file("equipment.json"))
	_equip_effects = _load(_data_file("equip_effects.json"))
	_skills = _load(_data_file("skills.json"))
	_cfg = _load(_data_file("combat.json"))
