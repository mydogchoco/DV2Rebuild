extends Node
## 콜로세움 **봇 vs 봇** 대량 시뮬레이터 — 밸런스 확인용(연출 없음, 순수 logic).
##
## 조립은 `fight.gd::_start` 를 그대로 따라간다(같은 결과를 내야 뜻이 있다):
##   무대 속성 추첨 → 양 팀 `PartyStats.summary_of(…, sel)` → `apply_passives` 양쪽
##   → `Battle.make_combatant`(스킬·각성·장비 효과 포함) → `Battle.simulate`.
##
## 매 판 **상대를 새로 만든다** — 명단에서 3마리를 고르고(4마리 랭커), 스킬 2칸을 다시 뽑고,
## 장비 강화를 다시 굴리고, 무대 속성을 다시 추첨한다. 실제 매치와 같은 분산이다.
##
## 사용(임시 오토로드로 등록해 실행):
##   --sim-a=<가드 연승|랭커 닉>  --sim-b=<…>  --sim-n=<판수>  --sim-mode=<team|single>
##   기본 = 선대군(999) vs 랭커 전원, 3vs3, 각 200판.
...
func _ready() -> void:
	...
	# `--sim-dump=1` = 양쪽 실 능력치를 한 번 찍는다(승률이 왜 그렇게 나오는지 보려고).
...
# --- 패배 원인 집계(`--sim-why=1`) -------------------------------------------
#
# "어떤 경우에 지는가"는 승률 한 줄로는 안 나온다. 판마다 갈리는 조건
# (무대 속성 · 상대 드래곤 · 양쪽이 뽑은 스킬 2칸)별로 **패배율**을 내야 어느 조건이
# 실제로 결과를 뒤집는지 보인다. `_all` = 그 조건이 나온 판 수, `_lose` = 그중 진 판.
var _all: Dictionary = {}
...
## `--sim-a/b` 값 → 상대 1기. 숫자면 그 연승의 **연승방지봇**, 아니면 **랭커 닉네임**.
...
## `--sim-skills=<드래곤>:<스킬>+<스킬>[;<드래곤>:…]` — **시트를 건드리지 않고** 장착 스킬만
## 갈아 끼워 본다("한울이 상처 파악 대신 빙결의 표식을 들면?" 류의 what-if 용).
## 레벨은 시트에 적힌 그 드래곤의 레벨을 그대로 쓴다(없으면 5).
...
func _battle(a_key: String, b_key: String, mode: String, rng: RandomNumberGenerator) -> Dictionary:
	...
	# 판마다 갈리는 조건을 결과에 붙여 둔다(`--sim-why=1` 집계용).
...
# --- 평타 판정 추적(`--sim-trace=1`) -----------------------------------------
#
# "크리 85면 거의 확정 아니냐"는 물음은 **판정 상한(prob_cap 70)** 과 상대 회피를 같이 봐야
# 답이 된다. 그래서 선대군 쪽 평타를 **맞은 상대별로** 세어 빗나감·크리 비율을 낸다.
var _atk: Dictionary = {}
...
## 한 진영의 실 능력치 1회 출력.
##
## ⚠️ **각성 스킬·장비 조건부 효과까지 얹은 뒤** 찍는다(`apply_passives`) — 전투가 보는 값과
##    같아야 한다. 종전엔 얹기 전 값을 찍어 한울의 크리가 10 으로 보였다(실제 전투값은 85).
##    무대 보정만 없다(추첨이라 판마다 다르다).
func _dump(key: String, mode: String, rng: RandomNumberGenerator) -> void:
	...
	var cap := Battle._crit_cap(Data.combat)      # 표시용 — 크리는 전용 상한(judge.crit_cap)
	for p: Dictionary in party:
		var st: Dictionary = p["stats"]
		# 각성·장비 효과는 스탯이 아니라 효과 목록으로 온다 — 전투가 쓰는 실효값으로 환산한다.
