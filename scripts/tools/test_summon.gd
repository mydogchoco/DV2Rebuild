...
func _logic() -> void:
	_say("[A] 규칙")
	var mat := {"uid": 1, "id": 101, "level": 30, "nickname": "치코"}
	var master := {"id": 101, "element": "fire", "art_id": 101}

	# 1) 기본 잠김 — 플래그가 없으면 무조건 불가
	_true("잠김이면 불가", Summon.plan(600, mat, master, false, 0).is_empty())

	# 2) 해금되면 계획이 나온다. 등급=최고 고정, 시간=그 등급의 표준 부화 시간
	var p := Summon.plan(600, mat, master, true, 0)
	...
	_true("이미 보유한 종 불가", Summon.plan(600, mat, master, true, 4).is_empty())
	...
	_true("일반 개체 OK", Summon.can_be_material(mat))
	_true("알 거부", not Summon.can_be_material({"id": 101, "egg": true}))
	_true("잠금 거부", not Summon.can_be_material({"id": 101, "locked": true}))
	_true("커스텀 600 거부", not Summon.can_be_material({"id": 600}))
	_true("커스텀 700 거부", not Summon.can_be_material({"id": 700}))
	_true("빈 개체 거부", not Summon.can_be_material({}))
...
func _integration() -> void:
	...
	# 재료용 임시 드래곤(끝에 정리한다)
	var mat_inst := UserDB.add_dragon(101, 20)
	...
	_true("잠김이면 계획 없음",
		Summon.plan(target, mat_inst, Data.get_dragon(101), false, UserDB.dex_step(target)).is_empty())

	# 해금 후 실행
	UserDB.set_pmeta(Summon.FLAG_UNLOCK, true)
	var p := Summon.plan(target, mat_inst, Data.get_dragon(101), true, UserDB.dex_step(target))
	...
	if not p.is_empty():
		...
		# 이제 같은 종은 더 못 만든다
		_true("상한 적용", Summon.plan(target, {"uid": 9, "id": 101, "level": 5},
			Data.get_dragon(101), true, UserDB.dex_step(target)).is_empty())
