...
func _tick() -> void:
	...
	if _step == 2:
		...
		# ⑤ 결과 워드아트 스파인 — success_type/failed_type 이 있고 글자 슬롯이 들어왔는가.
		...
		if not ResourceLoader.exists(rp):
			...
			# SUCCESS/FAILED 는 글자마다 슬롯이 있다(원작 워드아트가 스파인인 근거).
			...
			ri.free()
		# ⑥ 용액은 **확인 팝업**을 거친다 — 누르자마자 투입되면 안 된다.
		var pots: Array = (JSON.parse_string(FileAccess.open(
			"res://data/gems.json", FileAccess.READ).get_as_text())["upgrade"] as Dictionary)["potions"]
		...
		_save("potion_confirm.png")            # 팝업은 한 프레임 뒤에 그려진다
	elif _step == 26:
		...
		# 앞 단계에서 띄운 확인 팝업을 닫는다(안 닫으면 결과 화면을 덮는다).
		for c in _shop.get_children():
			if c is CanvasLayer:
				c.queue_free()
	elif _step == 30:
		# ⑦ 결과 화면 눈으로 — 원작 워드아트 스파인이 실제로 그려지는가(SUCCESS).
		# ⚠️ 원작 연출은 `DelayTime(0.2)` 뒤에 보이기 시작한다. `--script` 드라이버는 프레임이
		#    무제한이라 "N 프레임 뒤"로 세면 0.2초에 못 미친다 → **실제 시간**으로 기다린다.
		...
		create_timer(1.4).timeout.connect(func():
			...
			# 글자가 실제로 보이는 상태인가 — 워드아트 노드의 가시성으로 확인한다.
