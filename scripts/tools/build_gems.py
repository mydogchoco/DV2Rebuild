...
#
# 🟢 2026-08-01 — 세 재료의 정체가 위키 item.pdf p18 에서 확정됐다:
#   · `core` = **발록의 핵** — "업데이트를 통해 특수 장신구를 제작하거나 **상위 등급의
#     소울젬을 업그레이드** 하는 등 사용처가 늘어났다". 참조 프레임 `docs/ref/gem/소울젬4.png`
#     첫 칸(1/1)의 그림이 우리 `item_mtr/item_mtr_balrog_core` 와 픽셀 일치한다.
#   · `dust` = **마법가루** — "일반 젬을 분해하여 얻을 수 있는 마법 가루 … 혼성젬을 만들거나
#     **소울젬을 강화**할 때 사용된다". 축(공/방/체)에 따라 붉은/푸른/노란.
#   · `mat` = **카이저 발록의 파편 / 발톱 / 뿔** — "순서대로 상위 등급의 공격, 방어, 체력의
#     소울젬을 강화하는 데도 쓰이니 많이 모아두는 것이 좋다"(같은 쪽). 참조 프레임 셋째 칸의
#     어두운 뿔·발톱 그림과도 맞는다.
#
# ⚠️ 그런데 그 세 부위 아이템은 **우리 items.json 257종에 없다**(원작 후기 업데이트분 —
#    `발록의 피`·`파편`·`발톱`·`뿔` 전부 부재, 아이콘 프레임도 없다). CLAUDE.md §3 규칙상
#    아이콘 없는 아이템은 만들지 않는다.
#    🟦 사용자 확정 2026-08-01: **그 자리에 발록의 핵을 쓴다**(획득처는 나중에 특수 상점 =
#    원작 '방랑 상인'으로 구현 예정). 부위 아이템을 확보하면 이 표만 축별 키로 바꾸면 된다.
SOUL_MAT_ITEMS = {"dust": "", "mat": "balrog_core", "core": "balrog_core"}
SOUL_MAT_NOTE = (
    "위키 item.pdf p18 확정 — core=발록의 핵 · dust=마법가루(축별) · "
    "mat=카이저 발록의 파편/발톱/뿔(축별, 공/방/체 순). 부위 3종은 우리 items.json 에 없어 "
    "(원작 후기 업데이트분·아이콘 부재) 사용자 확정으로 발록의 핵을 대신 쓴다. "
    "같은 키가 두 칸에 걸리면 UI 가 한 칸으로 합친다."
)
...
def build() -> dict:
    ...
    return {
        ...
        "upgrade": {
            ...
            "_soul_mat_items_source": SOUL_MAT_NOTE,
