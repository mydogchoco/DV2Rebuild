extends SceneTree
# 일반공격 아트 3종 + 타입별 효과음 검증.
#   (1) scenes/fx/skill_1_{bite,scratch,hit}_spine.tscn 로드 · "animation" 존재 · 텍스처 연결
#   (2) 각 타입 효과음이 실제로 스트림으로 열리는지(임포트 누락이면 소리가 안 난다)
# Run: godot --headless --script res://scripts/tools/test_attack_fx.gd
const FX := [
